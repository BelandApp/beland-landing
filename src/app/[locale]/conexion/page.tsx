        ...
        {CAROUSEL_VIDEOS.map((video) => (
          <SwiperSlide key={video.id}>
            <div className="relative bg-transparent overflow-hidden rounded-2xl flex justify-center items-center w-full aspect-video min-h-[500px] sm:min-h-[600px] md:min-h-[700px] lg:min-h-[800px]">
              <iframe
                src={`https://player.vimeo.com/video/${video.src.split('/')[3].split('?')[0]}`}
...
export default function ConexionCircularPage() {
  const t = useTranslations("ConexionCircular");
        ...
        <section className="container mx-auto px-4 relative">
          <div className="text-center space-y-3 mb-12">
            <Badge className="text-sm px-4 py-1.5 bg-gradient-to-r from-primary/20 to-secondary/20 text-primary border-primary/30" variant="outline">
              <Sparkles className="w-3 h-3 mr-2 inline-block" /> {t("tag")}
            </Badge>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto mt-8">
              <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 backdrop-blur-sm">
                ...
                <div className="text-[10px] md:text-xs font-bold uppercase text-muted-foreground">{t("stats.totalRecycled")}</div>
              </div>
              <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 backdrop-blur-sm">
                <div className="text-2xl md:text-4xl font-black text-primary">{globalData.families}</div>
                <div className="text-[10px] md:text-xs font-bold uppercase text-muted-foreground">{t("stats.familiesHelped")}</div>
              </div>
              <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 backdrop-blur-sm">
                <div className="text-2xl md:text-4xl font-black text-primary">{globalData.impact.agua.toLocaleString()}</div>
                <div className="text-[10px] md:text-xs font-bold uppercase text-muted-foreground">{t("stats.waterSaved")}</div>
              </div>
              <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 backdrop-blur-sm">
                <div className="text-2xl md:text-4xl font-black text-primary">{globalData.impact.co2.toLocaleString()}</div>
                <div className="text-[10px] md:text-xs font-bold uppercase text-muted-foreground">{t("stats.co2Avoided")}</div>
              </div>
            </div>
          </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <Card className="border-2 border-primary/20">
                      <CardContent className="p-6">
                        <h4 className="font-bold mb-4 flex items-center gap-2">{t("events.resultsCard")}</h4>
                    ...
                    <Card className="border-2 border-secondary/20">
                      <CardContent className="p-6">
                        <h4 className="font-bold mb-4 flex items-center gap-2 text-secondary">{t("events.impactCard")}</h4>
                  ...
                  <Card className="bg-gradient-to-r from-primary/10 to-secondary/10 border-none">
                    <CardContent className="p-6 flex items-center justify-center gap-6">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-sm font-medium">{t("impact.heading")}</p>
                      </div>
                    </CardContent>
                  </Card>
                </div>
          ...
          <div className="text-center mt-12 py-4 border-t border-border">
            <span className="text-lg font-bold opacity-50 italic">14.02 - 01.10.2026</span>
          </div>
        </section> 
      </div>
      ...
      <section className="py-24 bg-white">
        <div className="container px-6 mx-auto">
          <div className="text-center mb-20">
            <Badge variant="outline" className="mb-4 border-slate-200 px-4 py-1">{t("challenge.recordBadge")}</Badge>
            <h2 className="text-5xl md:text-8xl font-black tracking-tighter leading-[0.9]">
              <span className="text-[#769C48]">{t("challenge.festivalTitle")}</span> <span className="text-orange-500">{t("challenge.circularTitle")}</span> <br />
              <span className="text-slate-900">{t("challenge.quitoTitle")}</span>
            </h2>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-16 max-w-6xl mx-auto items-start">
            <div className="flex-1 space-y-12">
              <div className="flex gap-6">
                ...
                <div>
                  <h4 className="text-2xl font-black uppercase italic mb-2">{t("challenge.heading")}</h4>
                  <p className="text-slate-600 text-lg leading-relaxed">{t("challenge.description")}</p>
                </div>
              </div>
              <div className="flex gap-6">
                ...
                <div>
                  <h4 className="text-2xl font-black uppercase italic mb-2">{t("challenge.sectionTitle")}</h4>
                  <p className="text-slate-600 text-lg leading-relaxed">{t("challenge.sectionDescription")}</p>
                </div>
              </div>
              ...
              <div className="bg-[#FAF9F6] p-10 rounded-[2.5rem] border border-slate-100 relative mt-16">
                ...
                <p className="mt-6 font-black text-[#769C48] uppercase tracking-widest text-sm">{t("challenge.quoteAuthor")}</p>
              </div>
            </div>
            ...
            <div className="flex-1 w-full lg:sticky lg:top-32">
              <div className="bg-white rounded-[3.5rem] p-10 md:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-slate-50 text-center space-y-10">
                <h3 className="text-3xl font-black text-slate-900">{t("impact.heading")}</h3>
                <div>
                  <span className="text-4xl font-black text-orange-500 leading-none">1</span>
                  <p className="uppercase font-black text-slate-400 tracking-tighter">{t("impact.recordLabel")}</p>
                </div>
                <div className="bg-orange-50/50 p-8 rounded-[2rem]">
                  <span className="text-4xl font-black text-orange-500 block">$1.8M+</span>
                  <p className="uppercase font-bold text-slate-500 text-xs mt-2 tracking-widest">{t("impact.incomeLabel")}</p>
                </div>
                <Link 
                ...
                >
                  <button className="w-full bg-[#769C48] text-white font-black py-6 rounded-2xl flex items-center justify-center gap-4 hover:bg-[#65853d] transition-all text-xl uppercase italic shadow-lg shadow-green-900/10 cursor-pointer">
                    {t("impact.cta")} <ArrowRight className="w-6 h-6" />
                  </button>
                </Link>
              </div>
            </div>
